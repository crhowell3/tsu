...
impl Diagnostic {
    pub fn is_for(&self, uri: &str) -> bool {
        let Some(ref related_information) = self.related_information else {
            return true;
        };

        related_information.iter().any(|ri| ri.location.uri == uri)
    }

    pub fn affected_lines(&self) -> Vec<usize> {
        let Range { start, end } = &self.range;
        crate::log!("Affected lines: {:?}", start.line..=end.line);
